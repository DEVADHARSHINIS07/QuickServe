    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
        <motion.div
          ...
          className="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden border border-slate-100 dark:border-slate-700 my-auto"
        >
          {/* Header Action Bar */}
          <div className="bg-slate-900 text-white p-3.5 sm:p-4 flex items-center justify-between no-print shrink-0">
            <div className="flex items-center gap-2">
              <span className="bg-amber-400 text-slate-900 font-bold px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs">
                Receipt
              </span>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <button
                ...
                className="p-1.5 sm:p-2 bg-amber-500 hover:bg-amber-600 text-slate-900 font-medium rounded-lg transition text-xs flex items-center gap-1"
          ...
          <div ref={printRef} className="p-4 sm:p-8 bg-amber-50/40 dark:bg-slate-800/50 print-content overflow-y-auto flex-1">
            {/* Canteen Branding Header */}
            <div className="text-center pb-6 border-b border-dashed border-amber-200 dark:border-slate-700">
              ...
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">{canteenConfig.canteenName}</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{canteenConfig.collegeName}</p>

              <div className="mt-4 inline-flex items-center gap-3 bg-white dark:bg-slate-700 px-4 py-2 rounded-2xl shadow-sm border border-amber-200/60 dark:border-slate-600">
                <div className="text-left">
                  <span className="text-[10px] uppercase tracking-wider text-slate-400 block font-semibold">Queue Number</span>
                  <span className="text-xl font-extrabold text-amber-600 dark:text-amber-400">{order.queueNumber}</span>
                </div>
                ...
                <div className="text-left">
                  ...
                  <span className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                    <Clock size={13} className="text-amber-500" /> {order.requestedReadyTime}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="py-4 border-b border-dashed border-amber-200 dark:border-slate-700 text-xs space-y-2">
              <div className="flex justify-between items-center text-slate-600 dark:text-slate-300">
                <span className="flex items-center gap-1.5"><User size={13} /> Student Name:</span>
                <span className="font-semibold text-slate-800 dark:text-slate-100">{order.studentName}</span>
              </div>
              <div className="flex justify-between items-center text-slate-600 dark:text-slate-300">
                <span>Student ID:</span>
                <span className="font-mono bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded text-slate-700 dark:text-slate-200">{order.studentId}</span>
              </div>
              <div className="flex justify-between items-center text-slate-600 dark:text-slate-300">
                <span>Date & Time:</span>
            ...
            {/* Food Items Table */}
            <div className="py-4 border-b border-dashed border-amber-200 dark:border-slate-700">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Order Items</h4>
              <div className="space-y-2.5">
                {order.items.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <span className={`w-2.5 h-2.5 rounded-full ${item.vegType === 'veg' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                      <span className="font-medium text-slate-800 dark:text-slate-200">{item.name}</span>
                      <span className="text-slate-400 font-semibold">x{item.quantity}</span>
                    </div>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">₹{item.price * item.quantity}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 space-y-2">
              ...
              <div className="flex justify-between text-base font-extrabold text-slate-900 dark:text-white pt-2 border-t border-slate-200 dark:border-slate-700">
                ...
                <span className="text-amber-600 dark:text-amber-400">₹{order.totalAmount}</span>
              </div>
            </div>

            {/* Status Stamp Badge */}
            <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-700/60 p-3 rounded-xl">
              <div className="flex items-center gap-2">
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 text-center no-print">
            <button
              onClick={onClose}
              className="w-full py-2.5 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl font-medium text-xs transition"
            >
              Close Receipt
            </button>
          </div>
